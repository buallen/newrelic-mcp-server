...
async function comprehensiveMallAnalysis() {
  ...
  try {
    ...
    console.log(`• Total mall-related log entries: ${generalResult.results.reduce((sum, r) => sum + (r.count || 0), 0)}`);
    ...
    if (anihonbuResult.results.length === 0) {
      console.log('🚨 CRITICAL: No anihonbu store processing detected during non-submission period');
