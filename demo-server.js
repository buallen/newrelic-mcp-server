...
const mockNRQLResults = {
  ...
  "campaign_performance": {
    results: [
      { timestamp: new Date(Date.now() - 60*60*1000).toISOString(), avg_duration: 1250, throughput: 120 },
      { timestamp: new Date(Date.now() - 30*60*1000).toISOString(), avg_duration: 2800, throughput: 95 },
...
class DemoNewRelicClient {
  ...
  async executeNRQL(query, accountId = null) {
    ...
    } else if (query.toLowerCase().includes('duration') || query.toLowerCase().includes('performance')) {
  ...
  try {
    switch (name) {
      ...
      case 'analyze-campaign-incidents':
        ...
        const errorData = await newRelicClient.executeNRQL("SELECT * FROM TransactionError WHERE appName LIKE '%campaign%' SINCE 1 day ago");
