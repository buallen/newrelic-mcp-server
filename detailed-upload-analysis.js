...
async function detailedUploadAnalysis() {
  ...
  try {
    ...
    console.log(`ParadigmMallKelanaJaya processing: ${mallSpecificResult.results[0]?.count || 0} entries`);
    ...
    const dates = ['2025-08-29', '2025-08-30', '2025-08-31', '2025-09-01', '2025-09-03', '2025-09-04', '2025-09-05', '2025-09-06', '2025-09-07'];
    ...
    for (const date of dates) {
      ...
      try {
        ...
        console.log(`  ${date}: Generated ${generated.toLocaleString()} | Processed ${processed} | Success: ${processed > 0 ? 'YES' : 'NO'}`);
      } catch (error) {
    ...
    console.log(`• Mall processing active: ✅ YES (${(mallSpecificResult.results[0]?.count || 0) > 0 ? 'Active' : 'Inactive'})`);
    console.log(`• File generation working: ✅ YES (${(datafilesResult.results[0]?.count || 0).toLocaleString()} files)`);
    console.log(`• FTP transmission logs: ${ftpDetailResult.results.length > 0 ? '✅ PRESENT' : '❌ MISSING'}`);
    console.log(`• Files moved to processed: ${processedResult.results.length > 0 ? '✅ YES' : '❌ NO'}`);
