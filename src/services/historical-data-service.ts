...
export interface HistoricalDataServiceInterface {
  ...
  getTimeSeriesData(entityId: string, metric: string, timeRange: TimeRange, interval?: string): Promise<TimeSeriesData>;
  getMultipleMetricsTimeSeries(entityId: string, metrics: string[], timeRange: TimeRange, interval?: string): Promise<TimeSeriesData[]>;
  ...
  // Data aggregation
  aggregateData(entityId: string, metric: string, timeRange: TimeRange, aggregationType: 'hourly' | 'daily' | 'weekly' | 'monthly'): Promise<DataAggregation>;
  
  // Comparison analysis
  compareTimePeriods(entityId: string, metric: string, baselinePeriod: TimeRange, comparisonPeriod: TimeRange): Promise<ComparisonAnalysis>;
  compareBetweenEntities(metric: string, entityIds: string[], timeRange: TimeRange): Promise<EntityComparison[]>;
  
  // Historical insights
  getHistoricalInsights(entityId: string, timeRange: TimeRange): Promise<HistoricalInsight[]>;
  identifyPerformancePatterns(entityId: string, timeRange: TimeRange): Promise<PerformancePattern[]>;
  
  // Data export
  exportHistoricalData(entityId: string, metrics: string[], timeRange: TimeRange, format: 'json' | 'csv'): Promise<string>;
}
...
export interface HistoricalInsight {
  type: 'performance_degradation' | 'improvement' | 'pattern_change' | 'anomaly_cluster' | 'seasonal_pattern';
  title: string;
  description: string;
...
export class HistoricalDataService implements HistoricalDataServiceInterface {
  ...
  async getTimeSeriesData(entityId: string, metric: string, timeRange: TimeRange, interval: string = '5 minutes'): Promise<TimeSeriesData> {
    try {
      ...
      this.logger.info('Retrieved time series data', { entityId, metric, pointCount: dataPoints.length });
  ...
  async getMultipleMetricsTimeSeries(entityId: string, metrics: string[], timeRange: TimeRange, interval: string = '5 minutes'): Promise<TimeSeriesData[]> {
    try {
      ...
      this.logger.info('Retrieved multiple metrics time series', { entityId, metricCount: metrics.length });
      return results;
    } catch (error) {
      this.logger.error('Failed to get multiple metrics time series', error, { entityId, metrics, timeRange });
      throw new Error(`Failed to get multiple metrics time series: ${error.message}`);
    }
  }

  async analyzeTrend(entityId: string, metric: string, timeRange: TimeRange): Promise<TrendAnalysis> {
    try {
  ...
  async detectAnomalies(entityId: string, metric: string, timeRange: TimeRange): Promise<AnomalyPoint[]> {
    try {
  ...
  async aggregateData(entityId: string, metric: string, timeRange: TimeRange, aggregationType: 'hourly' | 'daily' | 'weekly' | 'monthly'): Promise<DataAggregation> {
    try {
      ...
      this.logger.error('Failed to aggregate data', error, { entityId, metric, timeRange, aggregationType });
      throw new Error(`Failed to aggregate data: ${error.message}`);
    }
  }

  async compareTimePeriods(entityId: string, metric: string, baselinePeriod: TimeRange, comparisonPeriod: TimeRange): Promise<ComparisonAnalysis> {
    try {
      this.logger.info('Comparing time periods', { entityId, metric, baselinePeriod, comparisonPeriod });
      ...
      const changes = {
        averageChange: this.calculatePercentageChange(baselineAgg.summary.average, comparisonAgg.summary.average),
        minChange: this.calculatePercentageChange(baselineAgg.summary.min, comparisonAgg.summary.min),
        maxChange: this.calculatePercentageChange(baselineAgg.summary.max, comparisonAgg.summary.max),
        variabilityChange: this.calculatePercentageChange(baselineAgg.summary.stdDev, comparisonAgg.summary.stdDev)
      };
  ...
  async compareBetweenEntities(metric: string, entityIds: string[], timeRange: TimeRange): Promise<EntityComparison[]> {
    try {
      this.logger.info('Comparing between entities', { metric, entityIds, timeRange });
      
      const aggregations = await Promise.all(
        entityIds.map(entityId => 
          this.aggregateData(entityId, metric, timeRange, 'hourly')
        )
      );
  ...
  async getHistoricalInsights(entityId: string, timeRange: TimeRange): Promise<HistoricalInsight[]> {
    try {
        ...
        try {
          ...
          if (trendAnalysis.trendStrength > 0.7) {
            const insight: HistoricalInsight = {
              type: trendAnalysis.trend === 'increasing' ? 'performance_degradation' : 'improvement',
              ...
              severity: this.determineSeverityFromTrend(metric, trendAnalysis.trend, trendAnalysis.changeRate),
              timeRange,
          ...
          if (trendAnalysis.anomalies.length > 0) {
            const criticalAnomalies = trendAnalysis.anomalies.filter(a => a.severity === 'critical' || a.severity === 'high');
          ...
          this.logger.warn('Failed to analyze metric for insights', { entityId, metric, error: error.message });
        }
      }
      
      this.logger.info('Generated historical insights', { entityId, insightCount: insights.length });
      return insights;
    } catch (error) {
  ...
  async identifyPerformancePatterns(entityId: string, timeRange: TimeRange): Promise<PerformancePattern[]> {
    try {
      this.logger.info('Identifying performance patterns', { entityId, timeRange });
      
      const patterns: PerformancePattern[] = [];
      const timeSeriesData = await this.getTimeSeriesData(entityId, 'response_time', timeRange, '1 hour');
      ...
      this.logger.info('Identified performance patterns', { entityId, patternCount: patterns.length });
      return patterns;
    } catch (error) {
  ...
  async exportHistoricalData(entityId: string, metrics: string[], timeRange: TimeRange, format: 'json' | 'csv'): Promise<string> {
    try {
      this.logger.info('Exporting historical data', { entityId, metrics, timeRange, format });
      
      const timeSeriesData = await this.getMultipleMetricsTimeSeries(entityId, metrics, timeRange, '5 minutes');
      ...
      this.logger.error('Failed to export historical data', error, { entityId, metrics, timeRange, format });
  ...
  private calculateTrend(dataPoints: HistoricalDataPoint[]): { direction: 'increasing' | 'decreasing' | 'stable' | 'volatile', strength: number, changeRate: number } {
  ...
  private determineSignificance(changes: ComparisonAnalysis['changes']): 'high' | 'medium' | 'low' | 'none' {
  ...
  private generateComparisonInsights(changes: ComparisonAnalysis['changes'], significance: string): string[] {
    ...
    if (Math.abs(changes.averageChange) >= 10) {
      ...
      insights.push(`Average performance ${direction} by ${Math.abs(changes.averageChange).toFixed(1)}%`);
    }
    
    if (Math.abs(changes.variabilityChange) >= 20) {
      const direction = changes.variabilityChange > 0 ? 'increased' : 'decreased';
      insights.push(`Performance variability ${direction} by ${Math.abs(changes.variabilityChange).toFixed(1)}%`);
    }
    
    return insights;
  }

  private determineSeverityFromTrend(metric: string, trend: string, changeRate: number): 'info' | 'warning' | 'critical' {
  ...
  private detectDailyPattern(dataPoints: HistoricalDataPoint[]): { strength: number, examples: any[], impact: 'positive' | 'negative' | 'neutral' } {
    ...
    const examples = hourlyAverages.map((avg, hour) => ({
      timestamp: `${hour}:00`,
      value: avg,
      context: `Hour ${hour}`
  ...
  private detectWeeklyPattern(dataPoints: HistoricalDataPoint[]): { strength: number, examples: any[], impact: 'positive' | 'negative' | 'neutral' } {
    ...
    const examples = dailyAverages.map((avg, day) => ({
