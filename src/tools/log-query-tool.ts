...
// @ts-nocheck
import { MCPTool, MCPToolCallRequest, MCPToolCallResponse, MCPToolInputSchema } from '../types/mcp';
...
export class LogQueryTool implements MCPTool {
    ...
    try {
      ...
      switch (query_type) {
        ...
        case 'infrastructure_logs':
          nrqlQuery = this.buildInfrastructureLogsQuery(
            include_fields,
            time_period,
            limit,
            hostname
  ...
  private buildApplicationLogsQuery(
    fields: string[],
    timePeriod: string,
    limit: number,
    appName?: string,
  ...
  private buildInfrastructureLogsQuery(
    fields: string[],
    timePeriod: string,
    limit: number,
    hostname?: string
  ...
  static getQueryExamples(): Record<string, string> {
    return {
      recent_logs:
        'SELECT timestamp, message, hostname, level FROM Log SINCE 7 days ago ORDER BY timestamp DESC LIMIT 100',
      error_logs:
        "SELECT timestamp, message, hostname, level, apmApplicationNames FROM Log WHERE level IN ('ERROR', 'CRITICAL', 'FATAL') SINCE 7 days ago ORDER BY timestamp DESC LIMIT 50",
      application_logs:
        'SELECT timestamp, message, hostname, apmApplicationNames FROM Log WHERE apmApplicationNames IS NOT NULL SINCE 7 days ago ORDER BY timestamp DESC LIMIT 100',
      infrastructure_logs:
        "SELECT timestamp, message, hostname, agentName FROM Log WHERE agentName = 'Infrastructure' SINCE 7 days ago ORDER BY timestamp DESC LIMIT 100",
      ...
      log_count_by_application:
        'SELECT count(*) FROM Log WHERE apmApplicationNames IS NOT NULL FACET apmApplicationNames SINCE 7 days ago',
    };
  }
}
