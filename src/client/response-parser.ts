...
export class ResponseParser {
  ...
  parseNRQLResponse(graphqlResponse: GraphQLResult, originalQuery: string): NRQLResult {
    try {
      ...
      const performanceStats = this.parsePerformanceStats(
        nrqlData.performanceStats || {},
        results.length
      );
  ...
  parseGoldenMetrics(entities: any[]): any[] {
    return entities.map(entity => ({
      ...
      name: entity.name,
      metrics:
        entity.goldenMetrics?.metrics?.map((metric: any) => ({
          name: metric.name,
  ...
  parseAlertViolations(entities: any[]): any[] {
    return entities.map(entity => ({
      ...
      name: entity.name,
      violations:
        entity.alertViolations?.map((violation: any) => ({
